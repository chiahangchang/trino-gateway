...
|--------|----------|-------------|
| Add a cluster | Written immediately | Set to `PENDING` if active, `UNHEALTHY` if inactive |
| Update a cluster | Written immediately | Reset to `PENDING` if active, `UNHEALTHY` if inactive |
