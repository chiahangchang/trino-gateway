...
final class TestGatewayResource
{
            ...
            throws IOException
    {
        Request request = new Request.Builder()
                .url(format("http://localhost:%s/gateway/backend/activate/activate-test", routerPort))
    ...
    {
        Request request = new Request.Builder()
                .url(format("http://localhost:%s/gateway/backend/deactivate/deactivate-test", routerPort))
                .post(RequestBody.create("", JSON))
                .build();
        assertThat(httpClient.newCall(request).execute().isSuccessful()).isTrue();

        assertThat(getBackendState("deactivate-test").trinoStatus()).isEqualTo(TrinoStatus.UNHEALTHY);
    }
    ...
    {
        Request request = new Request.Builder()
                .url(format("http://localhost:%s/gateway/backend/activate/nonexistent", routerPort))
                .post(RequestBody.create("", JSON))
                .build();
        assertThat(httpClient.newCall(request).execute().code()).isEqualTo(404);

        Request stateRequest = new Request.Builder()
                .url(format("http://localhost:%s/api/public/backends/nonexistent/state", routerPort))
                .get()
                .build();
    ...
    {
        Request request = new Request.Builder()
                .url(format("http://localhost:%s/gateway/backend/deactivate/nonexistent", routerPort))
                .post(RequestBody.create("", JSON))
                .build();
        assertThat(httpClient.newCall(request).execute().code()).isEqualTo(404);
